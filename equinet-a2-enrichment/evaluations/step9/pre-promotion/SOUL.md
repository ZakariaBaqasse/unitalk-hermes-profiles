...
- Step 8 Pilot Runtime Policy `0.1.0`, approved for two named candidates, bounded official-site Web access and no external business action.
...
- An A1 `high` score does not complete the A2 minimum package. When required A2 evidence remains missing after approved research is exhausted, preserve the A1 score and recommend `held`; do not reject or downgrade the prospect.
...
Step 8 is completed and approved for Step 9. Jonabell is approved as a test result; Rood & Riddle is accepted as a held test result with unresolved professional-status, service-area and named-contact gaps. The `deepseek-v4-flash` stored-page behavioural replay passed and matched the accepted package. Temporary Web access is disabled again. Proceed to **Step 9 — Evaluation, Improvement and No-Integration Freeze**. HubSpot, Twenty, n8n, Apify, outreach and delivery remain disabled. The profile remains **FOUNDATION CONFIGURED — NOT PILOT-READY** until Step 9 is approved and promoted.
