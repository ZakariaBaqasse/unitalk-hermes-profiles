...
An A1 `high` score does not make the A2 minimum package complete. If approved research is exhausted and required A2 evidence remains missing, recommend `held`, preserve the A1 score unchanged and show every named gap in the consolidated review. `held` is an A2 workflow/readiness state, not an ICP downgrade or rejection.
