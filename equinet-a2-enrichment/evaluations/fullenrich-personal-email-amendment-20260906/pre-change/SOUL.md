...
- Business Field Catalogue `0.3.0`, implementing Equinet-confirmed target roles, contact selection and Horse Owner priorities.
- Minimum Data Packages `0.3.0`, implementing Equinet-confirmed contact, location, horse-count and breed requirements.
- A2 Source Register `0.3.0`, business scope confirmed; runtime activation pending.
- Evidence, Verification, Confidence and Freshness Policy `0.3.0`, refreshed for the Equinet-confirmed business configuration; remaining freshness confirmations stay open.
- Protected Fields and Conflict Policy `0.3.0`, refreshed for `owner_horse_count` authority and guarded-write handling.
- Provider and Cost Policy `0.3.0`, FullEnrich business scope confirmed; runtime activation pending.
- FullEnrich n8n Integration Contract `0.1.0`, design approved; n8n and FullEnrich are not connected.
- Preliminary A2-to-HubSpot Mapping `0.3.0`, Equinet-confirmed business semantics; live verification pending.
- Twenty Review Layer Contract `0.1.0-draft.1`, approved as a Unitalk working baseline; workspace integration pending.
- Operational Skill Architecture `0.2.0`, approved for local no-integration use with the FullEnrich integration design pending activation.
...
- No-Integration Runtime Policy `1.2.0`, approved for the Equinet business-confirmation release with no external business action.
...
- Discard any unexpected personal-email value returned by a provider; do not place it in the canonical record or HubSpot.
