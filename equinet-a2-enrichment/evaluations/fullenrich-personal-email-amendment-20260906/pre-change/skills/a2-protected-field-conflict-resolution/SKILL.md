...
- Existing populated HubSpot `Contact.owner_horse_count`: preserve as authoritative; a different explicit proposal becomes `preserve_and_hold`.
- Empty `owner_horse_count` on a new or matched record: a high-confidence exact proposal may become `propose_add` for final human review; no write occurs before the guarded HubSpot action and read-back.
- `horse_count_range`: ignore; never use it to decide or populate horse count.
