...
def normalise(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(data) > 1 and request.get("second_contact_reason") not in SECOND_CONTACT_REASONS:
        raise ValueError("second contact requires an approved reason")
    unexpected_personal_email_count = 0
    for raw in data:
        ...
        unexpected_personal_email_count += len(contact_info.get("personal_emails") or [])
        if isinstance(contact_info.get("most_probable_personal_email"), dict) and not contact_info.get("personal_emails"):
            unexpected_personal_email_count += 1
    ...
    result["result_count"] = len(result["contacts"])
    result["unexpected_personal_email_discarded_count"] = unexpected_personal_email_count
    return result
