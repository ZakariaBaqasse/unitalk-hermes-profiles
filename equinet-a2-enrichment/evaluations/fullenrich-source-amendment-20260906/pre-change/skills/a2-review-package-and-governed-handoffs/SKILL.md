...
The final review must show one selected contact by default, the reason for any second selected contact, and never more than two. If no suitable target-role person exists, show `Contact Needed / Needs Review`, preserve any general organisation contact separately and do not route outreach through an unrelated role. Missing enrichment data remains reviewable: show `approved_collect_during_discovery` as a human disposition while preserving the incomplete state and all open gaps.
