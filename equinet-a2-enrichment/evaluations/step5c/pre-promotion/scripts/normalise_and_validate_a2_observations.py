...
SOURCE_REGISTER = ROOT / "foundations/contracts/sources/a2-source-register-0.1.1-draft.1.json"
...
TRACKING_KEYS=*** "gclid", "mc_cid", "mc_eid"}
...
def normalise_value(field: dict[str, Any], observation: dict[str, Any]) -> Any:
    ...
    if key == "organisation.horse_count_band":
        raise ReviewRequired("horse-count band is derived only after the exact count is evidence-verified")
...
def validate_batch(request: dict[str, Any], catalogue: dict[str, Any], register: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, obs in enumerate(observations, 1):
        ...
        if availability != "available":
            ...
            if key == "organisation.horse_count_band" and fact_type != "derived_from_explicit_value":
                item_errors.append("horse-count band must be derived from an explicit value")
            try:
