...
def main():
 ...
 gap_case=next(x for x in f2['gap_cases'] if x['name']==('horse-owner-required-gap' if a.scenario=='horse_owner_requalification' else 'complete-farrier-no-research'));p=write('gap',gap_case['request']);gap_out=a.output_dir/'gap-plan.json';run('a2-build-gap-plan',['scripts/build_a2_gap_plan.py',str(p),'--output',str(gap_out)])
 snapshot=gap_case['request'];p=write('minimum-package',snapshot);min_out=a.output_dir/'minimum-package.json';run('a2-evaluate-minimum-package',['scripts/evaluate_a2_minimum_package.py',str(p),'--packages','foundations/contracts/business/a2-minimum-data-packages-0.1.1-draft.1.json','--output',str(min_out)])
