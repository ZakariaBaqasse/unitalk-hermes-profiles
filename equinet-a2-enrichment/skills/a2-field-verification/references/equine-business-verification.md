...
- For an existing matched HubSpot record, `Contact.owner_horse_count` is authoritative. For a net-new prospect, or when that property is empty, accept an exact horse count only from approved A1 evidence, recorded Equinet confirmation or the prospect-owned official website. Equinet has no separate first-party enrichment dataset.
