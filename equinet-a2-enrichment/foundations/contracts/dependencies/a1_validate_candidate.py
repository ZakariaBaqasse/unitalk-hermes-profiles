...
def cross_reference_errors(candidate: dict) -> list[str]:
    ...
    for index, contact in enumerate(candidate.get("public_contacts", [])):
        contact_key=(str(c...")), str(contact.get("value")).strip().lower())
    ...
    for check_name, result in candidate.get("duplicate_check", {}).items():
        ...
        if status in {"no_match", "possible_match", "confirmed_duplicate"}:
        ...
        if status in {"possible_match", "confirmed_duplicate"} and not matches:
            errors.append(f"duplicate_check.{check_name} status {status} requires at least one match.")
        if status == "no_match" and matches:
            errors.append(f"duplicate_check.{check_name} status no_match requires an empty matches list.")
