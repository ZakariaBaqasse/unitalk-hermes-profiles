...
The approved new-source order is prospect official website, FullEnrich and then separately gated Apify/HarvestAPI. FullEnrich People Search is role- and exact-domain-bounded with a default result limit of one and a maximum of two unique returned/retained people per prospect. People Lookup is used only for a known person whose identity, organisation or current role still needs verification. Contact Enrichment runs only for selected contacts and requests work email and mobile phone. Personal email is not requested by default; if returned incidentally, it is retained separately with provider provenance and verification status for human review and grants no consent, outreach or automatic CRM-write authority. n8n is the approved target orchestration layer and remains disconnected until its separate activation gate passes.
