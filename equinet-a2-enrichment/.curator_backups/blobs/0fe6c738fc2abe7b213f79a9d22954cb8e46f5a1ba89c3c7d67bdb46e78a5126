...
- Business Field Catalogue `0.3.1`;
- Minimum Data Packages `0.3.1`;
- Source Register `0.3.2`;
- Step 11 Website-First Implementation Manifest `0.1.0` for the locally implemented, live-gated website/Facebook path.

For the current integration-validation workflow, evaluate the bounded official website before new FullEnrich calls when a verified website exists. Preserve valid website Company contacts/socials and up to two named People regardless of role. When no selected current target remains, FullEnrich Search progresses through approved primary titles, exact generic `Owner`, validated reuse of a current A1-linked secondary Person with zero provider Search calls, then paid approved secondary titles only when no linked fallback is selected. Only after all valid stages return no retained Person may the record use `completed_no_target`.
