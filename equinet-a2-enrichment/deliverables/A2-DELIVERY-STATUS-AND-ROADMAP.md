...
**Next delivery gate:** `Step 8 — Bounded Real No-Integration Pilot Readiness`
...
## 3. Equinet decisions and remaining inputs

### Confirmed by Equinet and implemented in configuration `0.2.0`

- Farrier and Horse Owner target roles and priorities.
- One selected named contact by default and two maximum with a documented large-organisation or shared-responsibility reason.
- `Contact Needed / Needs Review` when no suitable target-role person is found; no unrelated-role fallback.
- Attempt both professional email and business phone; one verified channel is sufficient when the other is unavailable.
- Attempt complete address; state and country are the minimum.
- Horse Owner current role, stable/farm type, exact horse count and at least one verified breed are required for enrichment completeness.
- Existing matched HubSpot `owner_horse_count` is authoritative; net-new prospects may use explicit approved evidence; `horse_count_range` is ignored and new horse-count bands are not created.
- Missing required business data still proceeds to the consolidated human review and may be approved for collection during sales discovery.
...
### Step 3A — Business Field Catalogue — EQUINET CONFIRMED / VERSION 0.2.0 ACTIVE

- Active Equinet-confirmed configuration `0.2.0` contains 33 business fields.
- `person.public_profile_urls` and `organisation.public_profile_urls` are separate optional fields.
- Confirmed Farrier and Horse Owner target-role models included.
- Proposed field priorities use Required, Conditional Required, Optional and Do Not Collect.
- One contact is retained by default and at most two with a documented reason; an unrelated role cannot be used as fallback.
- Existing HubSpot `owner_horse_count` has authority when present; net-new explicit approved evidence remains permitted; `horse_count_range` is ignored and new bands are disabled.
...
- Equinet business confirmation is recorded in `foundations/decisions/A2-EQUINET-BUSINESS-CONFIRMATION-20260830.json`.

### Step 3B — Minimum Data Packages — EQUINET CONFIRMED / VERSION 0.2.0 ACTIVE

- Farrier package retains five verified core fields plus the confirmed named-contact path.
- Horse Owner package requires organisation, location, stable/farm type, exact horse count and at least one verified breed, plus a current target role and contact path.
- Both professional email and business phone are attempted; one verified channel is sufficient when the other is unavailable.
- No suitable target contact produces `Contact Needed / Needs Review`; a general organisation contact may be retained but does not satisfy the target-contact requirement.
- Missing Required fields produce a visible gap and incomplete record without automatic rejection or A1 score change.
- Material conflicts produce `needs_review` with a `held` workflow recommendation. Missing business information without a true blocking dependency proceeds to final review with an optional `approved_collect_during_discovery` disposition.
...
- Equinet confirmation is implemented in the active `0.2.0` package.
...
### Step 7 — Synthetic End-to-End Acceptance — COMPLETED / APPROVED
...
### Step 8 — Bounded Real No-Integration Pilot — COMPLETED / APPROVED FOR STEP 9
...
- Jonabell was approved as a Step 8 test result; Rood & Riddle was accepted as a held test result.
- The Rood & Riddle `deepseek-v4-flash` stored-page replay matched the accepted package with zero Web or external business actions.
- Temporary Step 8 Web access was disabled after the pilot.

### Step 9 — Evaluation, Improvement and No-Integration Freeze — COMPLETED / APPROVED
...
- Release `equinet-a2-no-integration-1.0.0` is promoted with Web disabled by default and all integration/write boundaries preserved.
...
- **Step 8 closed:** bounded real no-integration pilot accepted for Step 9; this is not production or contractual acceptance.
...
Proceed to **Step 10 — Integrations**. Start with read-only HubSpot metadata and permission verification; do not activate writes, outreach or automated provider use without their separate approvals and acceptance tests.

## 7. FullEnrich source-routing amendment

**Recorded:** `2026-09-06T13:07:00Z`  
...
- Contact Enrichment runs only for selected contacts and requests work email plus mobile phone. Personal email is not requested by default; if returned incidentally, it is retained separately with provenance and verification status for human review.
