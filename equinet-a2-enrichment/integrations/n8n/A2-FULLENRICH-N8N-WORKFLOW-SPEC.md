...
- Send exactly `contact.work_emails` and `contact.phones`; do not request `contact.personal_emails` by default.
- If FullEnrich nevertheless returns a personal email, retain it as `person.personal_email_candidate` with provider provenance and verification status for human review.
