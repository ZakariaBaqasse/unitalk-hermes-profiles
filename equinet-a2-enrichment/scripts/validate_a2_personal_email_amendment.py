#!/usr/bin/env python3
"""Validate the approved A2 incidental-personal-email amendment."""
...
def main() -> int:
    ...
    cat_path = ROOT / "foundations/contracts/business/a2-business-field-catalogue-0.3.1.json"
    min_path = ROOT / "foundations/contracts/business/a2-minimum-data-packages-0.3.1.json"
    source_path = ROOT / "foundations/contracts/sources/a2-source-register-0.3.1.json"
    evidence_path = ROOT / "foundations/contracts/evidence/a2-evidence-verification-confidence-freshness-policy-0.3.1.json"
    provider_path = ROOT / "foundations/contracts/governance/a2-provider-and-cost-policy-0.3.1.json"
    protected_path = ROOT / "foundations/contracts/governance/a2-protected-fields-and-conflict-policy-0.3.1.json"
    mapping_path = ROOT / "foundations/contracts/mappings/a2-hubspot-preliminary-mapping-0.3.1.json"
    integration_path = ROOT / "foundations/contracts/integrations/a2-fullenrich-n8n-integration-0.1.1.json"
    decision_path = ROOT / "foundations/decisions/A2-FULLENRICH-INCIDENTAL-PERSONAL-EMAIL-20260906.json"
    ...
    check("personal_email_incidental_retention_configured", fields["person.personal_email_candidate"]["collection_policy"] == "retain_if_incidentally_returned_by_approved_contact_enrichment" and all(v == "optional" for v in fields["person.personal_email_candidate"]["priority_by_segment"].values()) and fields["person.personal_email_candidate"]["paid_lookup_allowed"] is False)
    ...
    check("decision_scope_recorded", decision["decision"]["personal_email_requested_by_default"] is False and decision["decision"]["incidental_personal_email_retained"] is True)
    ...
    incidental_personal = {**valid_enrich, "field_key": "person.personal_email_candidate", "incidental_provider_output": True}
    check("incidental_personal_email_preflight_passes", preflight_module.preflight(incidental_personal, source, cat, provider)["preflight_status"] == "allowed_local_fixture")
    direct_personal = {**valid_enrich, "field_key": "person.personal_email_candidate"}
    check("personal_email_requires_incidental_marker", "personal_email_must_be_incidental_not_requested" in preflight_module.preflight(direct_personal, source, cat, provider)["blocks"])
    ...
    personal_only = copy.deepcopy(farrier_mobile_only)
    personal_only["fixture_id"] = "SYN-FE-PERSONAL-EMAIL-NON-CONTACTABILITY"
    personal_only["field_states"]["person.mobile_phone"] = "not_found"
    personal_only["field_states"]["person.personal_email_candidate"] = "verified"
    personal_minimum_result = minimum_evaluator.evaluate(personal_only, minimum)
    check("personal_email_does_not_satisfy_professional_contactability", personal_minimum_result["package_status"] == "incomplete" and "person.mobile_phone" in personal_minimum_result["unsatisfied_contact_requirements"])
    ...
    check("incidental_personal_email_retained_and_extra_profile_fields_removed", enrich_normalised["contacts"][0]["personal_email_candidate"] == "private@example.test" and enrich_normalised["contacts"][0]["personal_email_status"] == "DELIVERABLE" and enrich_normalised["incidental_personal_email_retained_count"] == 1 and "Example skill" not in serialised_enrich and "This must not be retained" not in serialised_enrich)
    ...
    result = {
        "record_type": "a2_fullenrich_incidental_personal_email_technical_validation",
