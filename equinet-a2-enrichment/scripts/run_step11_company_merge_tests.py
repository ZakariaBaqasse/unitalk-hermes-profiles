...
class Tests(unittest.TestCase):
 ...
 def test_catch_all_email_and_status_are_written_together(self):
  proposal={'run_id':'x','company_id':'c','company_final_status':'PARTIALLY_ENRICHED','resolved_people':[{'source_kind':'new','selection_stage':'website','website_person_id':'wp','full_name':'Alex Smith','work_email':{'value':'alex@catchall.example','status':'CATCH_ALL','provider_status':'CATCH_ALL','source':'fullenrich_contact_enrichment'},'mobile_phone':{'value':'+15550101','region':'US'},'statuses':{'identity':'VERIFIED','role':'CURRENT_AT_COMPANY','priority':'UNKNOWN','company_match':'CONFIRMED','enrichment':'ENRICHED'},'field_decisions':{}}]}
  with tempfile.TemporaryDirectory() as td:
   td=Path(td);inp=td/'p.json';out=td/'w.json';inp.write_text(json.dumps(proposal));p=subprocess.run([sys.executable,str(ROOT/'scripts/build_twenty_enrichment_write_plan.py'),str(inp),'--output',str(out)],cwd=ROOT,capture_output=True,text=True);self.assertEqual(p.returncode,0,p.stdout+p.stderr);fields=json.loads(out.read_text())['operations'][0]['fields'];self.assertEqual(fields['emails']['primaryEmail'],'alex@catchall.example');self.assertEqual(fields['emailStatus'],'CATCH_ALL')
 def test_provider_email_cannot_be_discarded_or_lose_status(self):
  base={'run_id':'x','company_id':'c','company_final_status':'PARTIALLY_ENRICHED','resolved_people':[{'source_kind':'existing','selection_stage':'website','twenty_person_id':'p1','full_name':'Alex Smith','work_email':{'value':'alex@catchall.example','source':'fullenrich_contact_enrichment'},'statuses':{'identity':'VERIFIED','role':'CURRENT_AT_COMPANY','priority':'UNKNOWN','company_match':'CONFIRMED','enrichment':'ENRICHED'},'field_decisions':{'work_email':'reject_invalid'}}]}
  with tempfile.TemporaryDirectory() as td:
   td=Path(td);inp=td/'p.json';out=td/'w.json';inp.write_text(json.dumps(base));p=subprocess.run([sys.executable,str(ROOT/'scripts/build_twenty_enrichment_write_plan.py'),str(inp),'--output',str(out)],cwd=ROOT,capture_output=True,text=True);self.assertEqual(p.returncode,1);self.assertTrue(any('must be retained' in error for error in json.loads(out.read_text())['errors']))
 def test_missing_provider_status_writes_unknown(self):
  proposal={'run_id':'x','company_id':'c','company_final_status':'PARTIALLY_ENRICHED','resolved_people':[{'source_kind':'new','selection_stage':'website','website_person_id':'wp','full_name':'Alex Smith','work_email':{'value':'alex@unknown.example','status':'unknown','provider_status':None,'status_source':'a2_missing_provider_status_fallback','source':'fullenrich_contact_enrichment'},'statuses':{'identity':'VERIFIED','role':'CURRENT_AT_COMPANY','priority':'UNKNOWN','company_match':'CONFIRMED','enrichment':'PARTIAL'},'field_decisions':{}}]}
  with tempfile.TemporaryDirectory() as td:
   td=Path(td);inp=td/'p.json';out=td/'w.json';inp.write_text(json.dumps(proposal));p=subprocess.run([sys.executable,str(ROOT/'scripts/build_twenty_enrichment_write_plan.py'),str(inp),'--output',str(out)],cwd=ROOT,capture_output=True,text=True);self.assertEqual(p.returncode,0,p.stdout+p.stderr);fields=json.loads(out.read_text())['operations'][0]['fields'];self.assertEqual(fields['emails']['primaryEmail'],'alex@unknown.example');self.assertEqual(fields['emailStatus'],'unknown')
 def test_email_status_is_reconciled_with_person_write(self):
  plan={'company_id':'c','operations':[{'operation_id':'person','operation_type':'update_person','twenty_person_id':'p1','fields':{'emails':{'primaryEmail':'alex@catchall.example','additionalEmails':[]},'emailStatus':'CATCH_ALL'}},{'operation_id':'final','operation_type':'update_company_status','twenty_company_id':'c','fields':{'a2EnrichmentStatus':'PARTIALLY_ENRICHED'}}],'expected_read_back':{'company_fields':{'a2EnrichmentStatus':'PARTIALLY_ENRICHED'},'people':[{'twenty_person_id':'p1','fields':{'emails':{'primaryEmail':'alex@catchall.example','additionalEmails':[]},'emailStatus':'CATCH_ALL'}}]}}
  state={'id':'c','people':[{'id':'p1','emails':{'primaryEmail':'old@example.com','additionalEmails':[]},'emailStatus':None}],'a2EnrichmentStatus':'PROCESSING'};old_env,old_http=push.twenty_env,push.http_json
  def fake(method,url,token,body=None,retries=0):
   if method=='PATCH' and '/people/p1' in url:state['people'][0].update(body or {});return {'data':{'person':dict(state['people'][0])}},{'http_status':200}
   if method=='PATCH':state.update(body or {});return {'data':{'company':dict(state)}},{'http_status':200}
   return {'data':{'company':dict(state)}},{'http_status':200}
  try:
   push.twenty_env=lambda:('http://twenty.invalid','token');push.http_json=fake;result=push.apply(plan,True)
  finally:push.twenty_env,push.http_json=old_env,old_http
  self.assertEqual(result['status'],'reconciled',result.get('mismatches'));self.assertEqual(state['people'][0]['emailStatus'],'CATCH_ALL')
