    ...
    for lead in leads:
        ...
        if lane == "no_website":
            no_site_overlays.append(not_scored_overlay(
                fingerprint,
                website_status="none_found" if status == "absent" else "error",
                enrichment_status="not_required_no_website" if status == "absent" else "failed",
                reason=None if status == "absent" else "invalid_website_candidate",
            ))
...
def validate_overlay(overlay: dict[str, Any]) -> None:
    ...
    if score_status == "SCORED":
        ...
        if website_status == "verified":
            if official_url is None or enrichment_status != "completed":
                raise ValueError("verified NOT_SCORED overlay requires a completed verified official website")
...
def build_website_overlays(discovery: dict[str, Any], state: dict[str, Any]) -> dict[str, Any]:
    ...
    if state.get("schema_version") != "a1.website-enrichment-state.v1":
        raise ValueError("Expected a1.website-enrichment-state.v1")
    ...
    for lead in leads:
        ...
        if status == "scored":
            ...
            overlay = not_scored_overlay(
                ...
                website_status="verified",
                enrichment_status="completed",
                reason="verified_site_unscored_missing_pipeline_inputs",
            ...
            overlay = not_scored_overlay(fingerprint, website_status="none_found", enrichment_status="failed", reason="official_website_not_found")
        elif status == "failed":
            history = record.get("failure_history") if isinstance(record.get("failure_history"), list) else []
            reason = optional_text(history[-1].get("error")) if history and isinstance(history[-1], dict) else "enrichment_failed"
...
def processing_next_action(discovery: dict[str, Any], work_dir: Path) -> dict[str, Any]:
    ...
    if not routing_path.is_file():
        ...
        if final_path.is_file():
            ...
            if state.get("schema_version") != "a1.website-enrichment-state.v1" or state.get("run", {}).get("run_id") != run_id:
                raise ValueError("Enrichment state is invalid or belongs to another run")
            records = state.get("leads") if isinstance(state.get("leads"), dict) else {}
            ...
            ready = any(
                ...
                and isinstance(record.get("pipeline_inputs"), dict)
                and all(key in record["pipeline_inputs"] for key in ("classification_decision", "qualification_assessment", "confidence_assessment"))
                for record in records.values() if isinstance(record, dict)
            )
            incomplete = any(
                record.get("status") == "completed"
                and (record.get("enrichment") or {}).get("website_status") not in {"none_found", "verified"}
                and not (
                    isinstance(record.get("pipeline_inputs"), dict)
                    and all(key in record["pipeline_inputs"] for key in ("classification_decision", "qualification_assessment", "confidence_assessment"))
                )
                for record in records.values() if isinstance(record, dict)
            )
            terminal = bool(statuses) and all(
                status in {"scored", "failed"}
                or (status == "completed" and (record.get("enrichment") or {}).get("website_status") in {"none_found", "verified"})
                for status, record in ((record.get("status"), record) for record in records.values() if isinstance(record, dict))
            )
            if pending:
                ...
                action = "operator_repair_missing_pipeline_inputs"
            elif not terminal:
