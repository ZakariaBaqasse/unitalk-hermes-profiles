...
valid HTTP(S) website candidate present
  ...
  → if the official site is verified but assessment inputs remain incomplete, stage an explicit verified-unscored overlay that retains the official URL
...
Website verification or enrichment must be bounded and resumable. If verification is unsuccessful, access is blocked or retries are exhausted, preserve the failure artifact and fall back to unscored staging without `domainName`. If official-site verification succeeds but a downstream assessment wrapper cannot complete, preserve the cited official URL in overlay v2 and fall back to unscored staging with `domainName`. Person creation still follows the immutable `name` field and does not depend on website verification or scoring.
