...
SCHEMA_VERSION = "equinet.n8n-poll-ticket.v1"
...
def validate_ticket(ticket: dict[str, Any]) -> None:
    ...
    extra = set(ticket) - REQUIRED_KEYS
    if missing:
    ...
    if ticket["schema_version"] != SCHEMA_VERSION:
        raise ValueError(f"unsupported schema_version: {ticket['schema_version']!r}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "init":
        ...
        ticket = load_ticket(path)
        result = {"next_action": next_action(ticket), "ticket": ticket}
