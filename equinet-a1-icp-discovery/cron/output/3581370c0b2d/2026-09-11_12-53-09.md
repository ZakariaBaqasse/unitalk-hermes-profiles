...
Processing is blocked before routing.

- Ticket: `A1-20260911124516-B16386`
- n8n result was already retrieved once and is valid at the envelope level:
  - Contract: `a1.discovery-result.v1`
  - Returned leads: 4; summary `returned`: 4
  - Source policy version: `2.0.0` (accepted)
- Router preflight failed with:

```text
RuntimeError: jsonschema is required to validate enrichment state v2
```

The required `python3` runtime lacks the `jsonschema` dependency, so `route_and_overlay.py next-action` cannot validate the state or safely determine the durable processing phase. No routing, enrichment, Twenty staging, ticket consumption, or delivery marker was performed.

Result artifact remains preserved at:

```text
runtime/n8n-poll-tickets/A1-20260911124516-B16386-discovery-result.json
```

Operator repair is required to provision the approved runtime dependency outside the discovery run, then resume from the persisted ticket/result without rerunning n8n or retrieving its result again.
