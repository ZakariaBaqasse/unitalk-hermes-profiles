...
**Operator repair required — routing is blocked before any new staging action.**

- Ticket: `A1-20260911124516-B16386`
- Discovery result was already retrieved exactly once; SHA-256: `d826a33eafed76959c581f11c1e021a64be5be74fd5fab5921bad8173642d2a8`.
- The compact result is contract-valid (`a1.discovery-result.v1`), uses accepted source policy `2.0.0`, and contains four leads.

**Blocker:**  
`route_and_overlay.py next-action` failed with:
...
```text
runtime/n8n-poll-tickets/A1-20260911124516-B16386-operator-repair-router-runtime.json
```

No n8n rerun or re-fetch occurred. No new Twenty write, website assessment, or score was attempted in this cron run. The approved PATH-selected Python runtime must be provisioned with `jsonschema` outside the discovery run, then processing can resume from the immutable retrieved result and existing `A1-20260911124516-B16386-two-lane-v1` work directory.
