...
Operator repair required — post-retrieval routing cannot proceed.

- **Ticket:** `A1-20260911124516-B16386`
- **Run:** `A1-20260911124413-CG2JSE`
- **Blocker:** `missing_python_jsonschema_dependency`
- **Failed deterministic step:** `route_and_overlay.py next-action`
- **Error:** `jsonschema is required to validate enrichment state v2`
- **Artifact:** `runtime/n8n-poll-tickets/A1-20260911124516-B16386-operator-repair-router-runtime.json`

The compact n8n result was already retrieved exactly once and retained. No n8n rerun/re-fetch, Twenty write, enrichment assessment, or scoring action was attempted in this cron run.

Required repair: provision `jsonschema` in the approved profile runtime used by PATH-selected `python3` **outside this discovery run**, then resume from deterministic router `next-action` using the immutable retrieved result and existing two-lane work directory.
