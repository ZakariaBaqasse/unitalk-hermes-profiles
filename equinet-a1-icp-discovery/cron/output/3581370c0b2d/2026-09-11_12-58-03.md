# Cron Job: equinet-n8n-monitor-A1-20260911124516-B16386

**Job ID:** 3581370c0b2d
**Run Time:** 2026-09-11 12:58:03
**Schedule:** every 2m
...
3. Build a configuration snapshot, then create the durable v2 ticket:

```text
python3 skills/system-operations/equinet-n8n-discovery-control/scripts/build_configuration_snapshot.py \
...
```text
python3 skills/post-n8n-public-website-enrichment/scripts/migrate_state_v1_to_v2.py \
...
Monitor Equinet n8n ticket /opt/data/profiles/equinet-a1-icp-discovery/runtime/n8n-poll-tickets/A1-20260911124516-B16386.json. This is a fresh cron session.
Use python3 with the full profile-relative skill paths below; do not invoke absolute interpreter binaries. Never call cronjob from this cron run and never rerun or modify n8n.

Run `python3 skills/system-operations/equinet-n8n-discovery-control/scripts/poll_ticket.py expire --file /opt/data/profiles/equinet-a1-icp-discovery/runtime/n8n-poll-tickets/A1-20260911124516-B16386.json` and then `next-action`.
...
Progress update — ticket remains in processing, not yet terminal.

- **n8n result:** retrieved previously and preserved once; source policy version `2.0.0` matches the active accepted runtime label.
- **Discovery result:** 4 unique leads total:
  - 1 no-website lead already staged through the no-website lane.
  - 3 website-candidate leads processed for official-site research.
- **Official-site research accepted:**  
  - Winter Moon Farm — verified official site; boarding-stable evidence retained.  
  - Razor Creek Stables — verified official site; full-board/stabling evidence retained.  
  - Kentucky HBPA — verified official site; association identity and public contact evidence retained.
- **Assessment:** Kentucky HBPA was evidence-classified `out_of_scope` because its verified site establishes an industry association, not an approved professional Farrier or commercial Horse Owner/equine-operation prospect type.
- **Remaining state:** Winter Moon Farm and Razor Creek Stables are both `assessment_pending`; no score, overlay, or website-lane Twenty write has been produced for either.
- **State artifact:**  
  `runtime/n8n-poll-tickets/A1-20260911124516-B16386-two-lane-v1/enrichment-state.json`

No outreach, A2 handoff, HubSpot write, or non-Company/Person Twenty action occurred. Human review remains mandatory.
