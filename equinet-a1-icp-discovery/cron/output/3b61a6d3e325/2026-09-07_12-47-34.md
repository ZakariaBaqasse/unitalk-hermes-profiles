...
**Operator repair required — staging has not begun.**

- n8n result was already retrieved once and is ready for processing.
- Routing completed: **30 leads** total — **28 no-website**, **2 website-candidate**.
- The mandatory Twenty pre-write capability gate failed before any Company or Person write.

**Persisted blocker:**  
`Twenty linked-Person duplicate-preflight company filter returned HTTP 400. The Company/Person API contract capability gate is not usable; no staging write was attempted.`

Artifact: `runtime/n8n-poll-tickets/two-lane-v1-execution-1282/twenty-capability-failure.json`

Required recovery: repair and regression-test the deterministic Twenty People company-filter query contract, then perform a separately controlled staging recovery from the immutable discovery result. Do **not** rerun n8n or retry any write.
