...
**Staging blocked — operator repair required.**

- n8n execution `1282` succeeded; its compact result was already retrieved and preserved.
- Routing completed: **30 leads** total — **28 no-website**, **2 website candidates**.
- No Twenty write was attempted.

**Blocker:** the Twenty linked-Person duplicate-preflight Company filter returned **HTTP 400**. Company authentication and Company duplicate filter forms succeeded, but the required People-by-Company filter is not usable.

**Required recovery:** repair and regression-test the deterministic Twenty People Company-filter query contract, then run a separately controlled staging recovery using the immutable discovery result. Do **not** rerun n8n or retry any uncertain write.

Artifact: `runtime/n8n-poll-tickets/two-lane-v1-execution-1282/twenty-capability-failure.json`
