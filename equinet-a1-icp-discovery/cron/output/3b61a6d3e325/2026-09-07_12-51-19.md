...
**Operator repair required — staging halted before any CRM write.**

Twenty’s read-only capability probe authenticated successfully, but the required linked-Person preflight filter failed:

- `people_company_filter`: HTTP **400** / unusable
- Company fingerprint, domain, and name filters: HTTP **200** / usable
- No Twenty Company or Person write was attempted.
- n8n was not rerun and its compact result was not retrieved again.

Persisted blocker artifact:

`runtime/n8n-poll-tickets/two-lane-v1-execution-1282/operator-repair-twenty-capability.json`

Repair and validate the deterministic Twenty People filter/serialization before resuming from the immutable discovery result.
