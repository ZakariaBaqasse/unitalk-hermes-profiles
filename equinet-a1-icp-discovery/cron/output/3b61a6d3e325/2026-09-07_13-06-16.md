# Cron Job: equinet-a1-execution-1282-monitor

**Job ID:** 3b61a6d3e325
**Run Time:** 2026-09-07 13:06:16
**Schedule:** every 2m
...
- One optional linked Person when the immutable n8n lead has `name`.
...
Monitor the Equinet n8n ticket at runtime/n8n-poll-tickets/execution-1282.json in this fresh cron session. Do not invoke cronjob and never re-run or modify n8n. Use python3 (PATH-selected) and the profile-relative skills paths.

First run `python3 skills/system-operations/equinet-n8n-discovery-control/scripts/poll_ticket.py expire --file runtime/n8n-poll-tickets/execution-1282.json`, then `next-action`. For poll_metadata, call n8n get_execution with workflowId 3tBw9IHcr4knwM4H, executionId 1282, includeData false, then persist status using observe. Return [SILENT] for unchanged non-terminal status.

If terminal success: claim result before retrieval. Retrieve exactly once with n8n get_execution includeData true, nodeNames ["Build Final Discovery Result"], truncateData 1. Persist only the compact result to runtime/n8n-poll-tickets/execution-1282-result.json, determine the actual application run ID from that result, replace the placeholder application_run_id in the ticket before mark-retrieved, compute SHA-256, and call mark-retrieved. Never fetch all-node data.

Then follow the production two-lane procedure deterministically: validate a1.discovery-result.v1; use work directory runtime/n8n-poll-tickets/two-lane-v1-execution-1282; route with route_and_overlay.py; stage no-website lane via stage_twenty_rest.py using required manifests and overlays; initialise/research website lane in stable batches up to five using only permitted official-site verification and cited enrichment; run deterministic validation/scoring where supported; build website overlays; stage website lane; merge indexes and only then mark-consumed. Do not repeat uncertain writes. Preserve no-site/failed-verification items as explicitly unscored and omit domainName before verification. Every named lead requires terminal linked Person disposition after Company verification. Any operator repair state must stop with its exact blocker. Do not initiate outreach, make HubSpot writes, invoke A2, or create CRM objects other than Company plus optional linked Person.

On terminal completion mark delivered with delivery reference local-cron and return a concise aggregate summary and final staging-index path; otherwise return [SILENT].

## Response

**Execution 1282 completed and delivered.**

- **n8n result:** retrieved once; ticket consumed and marked delivered (`local-cron`).
- **Routing:** 30 total leads — **28 no-website**, **2 website candidates**.
- **Website lane:** both official websites were verified with cited official-site evidence. Both were staged as **verified-unscored**: neither represented a confirmed in-scope professional Farrier or commercial Horse Owner for deterministic ICP assessment.
- **Scored:** 0. **Fallback/unscored:** 30.

**Twenty staging aggregate**
- **29** `company_staged_person_sync_failed`
  - Company and linked Person were created/read-back reconciled, including verified Company–Person relations.
  - Terminal limitation: `company_contact_migration_failed`.
- **1** `company_sync_failed`
  - Reason: `twenty_readback_reconciliation_failed`.
  - Its linked Person is `blocked_by_company`.
- **Possible matches:** 0.
- **Missing-name blocks:** 0.

The final combined index is complete and covers all original fingerprints:

`runtime/n8n-poll-tickets/two-lane-v1-execution-1282/final-staging-index.json`

HubSpot remained read-only. No outreach, A2 handoff, Opportunities, Tasks, campaigns, or messages were created. All records remain subject to human review.
