...
- n8n result: already retrieved; routing completed.
- Routed leads: **28 no-website**, **2 website-candidate** (30 total).
- Blocking check: `stage_twenty_rest.py --check-connection`
  - Authentication and Company filters succeeded.
  - Linked-Person duplicate-preflight filter failed: `people_company_filter` returned **HTTP 400**.
  - Result: `api_contract_usable: false`.
- No Twenty Company or Person writes were attempted, and no discovery, HubSpot, A2, or outreach action was performed.

The deterministic Twenty People company-filter compatibility must be repaired and regression-tested before resuming from the immutable result at:

`runtime/n8n-poll-tickets/execution-1282-result.json`
