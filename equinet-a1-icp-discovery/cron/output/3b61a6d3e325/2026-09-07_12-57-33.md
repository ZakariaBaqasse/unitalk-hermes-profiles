...
**Operator repair required — Twenty staging capability gate failed before any write.**

- Ticket: `execution-1282`
- n8n result: already retrieved once; lifecycle `succeeded`
- Routing completed: **30** leads  
  - No website candidate: **28**  
  - Website candidate: **2**
- Blocking step: required Twenty read-only connection probe before no-website staging.
- Probe result: authentication and Company filter checks succeeded, but linked-Person duplicate-preflight filter failed:
  - `people_company_filter`: **HTTP 400**
  - `api_contract_usable`: `false`

No Twenty Company or Person writes were attempted, no HubSpot writes occurred, and no website-lane enrichment or scoring was started. The ticket remains unconsumed and undelivered.

Repair the Twenty People Company-filter compatibility issue in the deterministic staging worker/API configuration, then resume from the immutable routed artifacts at:

`runtime/n8n-poll-tickets/two-lane-v1-execution-1282/`
