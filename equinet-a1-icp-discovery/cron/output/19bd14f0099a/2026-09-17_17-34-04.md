...
## Equinet A1 discovery run completed — Weatherford, Texas Farriers

- **n8n execution:** `1383` succeeded.
- **Application run:** `A1-20260917172359-W8QME6`
- **Result retrieval:** claimed and retrieved once; ticket is **consumed and delivered**.
- **Final staging index:**  
  `runtime/n8n-poll-tickets/weatherford-farrier-1383/two-lane-v1/final-staging-index.json`
...
|---|---:|
| Returned leads | 9 |
| No website candidate | 6 |
| Website candidates | 3 |
| Website terminal verification failure | 1 |
| Website evidence-backed out of scope | 2 |
| Scored ICP candidates | 0 |

Website findings:
- **Double C Farrier Service / James Cockrell:** the only candidate destination was `facebook.com`, which is blocklisted. Verification was not attempted; staged as an unscored pre-verification failure without an official domain.
- **Peak Performance Equine Hospital / Kyler Richard Gilbreath:** official site verified, but the supplied individual’s association and an approved Farrier/Horse Owner operating identity were not established. Classified **out of scope**.  
  Evidence: `https://peakperformanceequinehospital.com/`, `https://peakperformanceequinehospital.com/contact-us`
- **Parker County Farrier Supply 2.0:** official site verified as a farrier-products supplier, not as a professional farrier or horse-managing equine operation. Classified **out of scope**.  
  Evidence: `http://www.dfwfarrier.com/`

### Twenty staging

- **Companies:** 9/9 have terminal staged Company dispositions with verified Company IDs and reconciliation artifacts.
- **People required:** 8
  - **7** linked People have verified `companyId` relations and reconciliation artifacts.
  - **1** linked Person has a terminal `sync_failed` disposition: `twenty_person_readback_reconciliation_failed` for fingerprint `phone:17725302361`. The Person create was attempted and returned an ID, but read-back/relation reconciliation was not verified. The write outcome is explicitly **not uncertain**; no retry was performed.
- The remaining 8 Company statuses report `company_staged_person_sync_failed` because Company-contact migration failed after the linked-Person flow. Company and Person staging/relation artifacts remain preserved where verified.

### Important limitations and controls

- n8n’s HubSpot screening remained **read-only**. It reported **6 HubSpot check failures** and six leads without a company-domain lookup key; no broader HubSpot checks were performed.
- No HubSpot records were written.
- No Twenty objects outside Companies and linked People were created.
- **No outreach, campaigns, messages, Opportunities, Tasks, or A2 handoff occurred.**
- All staged records remain subject to **A1 Business Reviewer** human review.
